...
class DataFragment : androidx.fragment.app.Fragment(),
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        catParams = CoordinatorLayout.LayoutParams(
            NumberFormatter.dp2px(
                context!!,
                120F
            ), NumberFormatter.dp2px(context!!, 120F)
        )

        /**
         * 测量@layout_data_page的高度和宽度
         */
        layout_data_page.viewTreeObserver.addOnGlobalLayoutListener(object :
        ...
        layout_data_month_overview.setOnTouchListener { v, event ->
            ...
            when (event.action) {
                ...
                MotionEvent.ACTION_UP -> {
                    if (catR > layoutWidth - NumberFormatter.dp2px(
                            context!!,
                            80F
    ...
    override fun onChildClick(
        ...
        this.fragmentManager?.let { detail.show(it, "BDF") }
        return true
    }
    ...
    override fun onDismiss(dialog: DialogInterface?) {
        ...
        try {
            ...
            if (adapter.getChildrenCount(cGroupP) <= 1) {
                ...
                when (a.ioType) {
                    IOType.Outcome -> {
                        adapter.daySumList[cGroupP][IOType.Outcome] =
                            adapter.daySumList[cGroupP][IOType.Outcome]!! - a.amount
                    }
                    IOType.Income -> {
                        adapter.daySumList[cGroupP][IOType.Income] =
                            adapter.daySumList[cGroupP][IOType.Income]!! - a.amount
                    }
                    else -> {
                    }
                }
...
//                Log.d(TAG, "First POS: $firstVisibleItem, GROUP: $groupPosition, CHILD: $childPosition")
                if (firstGroup.date != null && adapter.getGroupDate(groupPosition) != firstGroup.date) {
                    //TODO(执行月份变化的判断)
                    if (firstGroup.position != null && adapter.getGroupDate(groupPosition) != null && !DateConverter.equalMonth(
                            firstGroup.date!!,
                            adapter.getGroupDate(groupPosition)!!
                        )
                    ) {
    ...
    private fun onMonthChange() {
        ...
        val info = BillRecordProvider.joinGroupByMonth(
            start.time,
            end.time,
            BYIOHelper(context!!).getBill(start.time, end.time)
        )
        data_month_outcome.text =
            "${resources.getString(R.string.outcome)}: ${NumberFormatter.decimalFormat2.format(info[0].outcomeSum)}"
