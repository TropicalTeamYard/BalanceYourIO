...
class BillRecordProvider(var context: Context) {
    ...
    companion object {
        fun joinGroupByDay(data: ArrayList<BillRecord>): ArrayList<BillRecordUnit> {
            ...
            if (data.isNotEmpty()) {
                start = DateConverter.cutToDate(data.last().time!!)
                    ...
                    if (DateConverter.cutToDate(data.first().time!!) > end) DateConverter.cutToDate(
                        data.first().time!!
                    ) else end
            }
        ...
        fun joinGroupByDay(
            ...
            data: ArrayList<BillRecord>,
            sort: Boolean = true
        ): ArrayList<BillRecordUnit> {
            if (sort)
                data.sortByDescending { it.time }
        ...
        fun joinGroupByWeek(data: ArrayList<BillRecord>): ArrayList<BillRecordUnit> {
            ...
            if (data.isNotEmpty()) {
                start = DateConverter.cutToWeek(data.last().time!!)
                    ...
                    if (DateConverter.cutToWeek(data.first().time!!) > end) DateConverter.cutToWeek(
                        data.first().time!!
                    ) else end
            }
        ...
        fun joinGroupByMonth(data: ArrayList<BillRecord>): ArrayList<BillRecordUnit> {
            ...
            if (data.isNotEmpty()) {
                start = DateConverter.cutToMonth(data.last().time!!)
                    ...
                    if (DateConverter.cutToMonth(data.first().time!!) > end) DateConverter.cutToMonth(
                        data.first().time!!
                    ) else end
            }
        ...
        fun joinGroupByYear(data: ArrayList<BillRecord>): ArrayList<BillRecordUnit> {
            ...
            if (!data.isEmpty()) {
                start = DateConverter.cutToYear(data.last().time!!)
                    ...
                    if (DateConverter.cutToYear(data.first().time!!) > end) DateConverter.cutToYear(
                        data.first().time!!
                    ) else end
            }
