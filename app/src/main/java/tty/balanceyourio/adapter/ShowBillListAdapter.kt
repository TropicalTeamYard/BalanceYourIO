...
class ShowBillListAdapter(var context: Context) : BaseExpandableListAdapter() {
                ...
                for (j in 0 until allBillRecord.size) {
                    if (DateConverter.equalDate(allBillRecord[j].time!!, dateList[i])) {
                        ...
                        when (allBillRecord[j].ioType) {
                            IOType.Income -> daySumList[i][IOType.Income] =
                                daySumList[i][IOType.Income]!! + allBillRecord[j].amount
                            IOType.Outcome -> daySumList[i][IOType.Outcome] =
                                daySumList[i][IOType.Outcome]!! + allBillRecord[j].amount
                            else -> daySumList[i][IOType.Unset] =
                                daySumList[i][IOType.Unset]!! + allBillRecord[j].amount
                        }
                    }
                }
    ...
    override fun getGroupView(
        ...
        if (convertView == null) {
            view = LayoutInflater.from(this.context)
                .inflate(R.layout.item_show_bill_parent, parent, false)
        ...
        viewHolder.outcome.text =
            "${context.resources.getString(R.string.outcome)} ${decimalFormat.format(daySumList[groupPosition][IOType.Outcome])}"
        return view
    }
    ...
    override fun getChildView(
        ...
        if (convertView == null) {
            view = LayoutInflater.from(this.context)
                .inflate(R.layout.item_show_bill_child, parent, false)
