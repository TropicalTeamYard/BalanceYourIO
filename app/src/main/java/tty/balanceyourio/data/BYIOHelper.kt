...
class BYIOHelper(context: Context) : SQLiteOpenHelper(context, DB_NAME, null, DB_VERSION) {
    ...
    fun getBill(): ArrayList<BillRecord> {
        ...
        if (cursor.moveToFirst() && cursor.count > 0) {
            do {
                ...
                billRecord.time =
                    simpleDateFormat.parse(cursor.getString(cursor.getColumnIndex("time")))
    ...
    fun getBill(from: Date, to: Date): ArrayList<BillRecord> {
        ...
        val cursor = readableDatabase.rawQuery(
            "select * from $NAME_BILL where datetime(time) between datetime(?) and datetime(?)",
    ...
    fun getBill(id: Int): BillRecord {
        ...
        val cursor =
            readableDatabase.rawQuery("select * from $NAME_BILL where _id = ?", arrayOf("$id"))
        if (cursor.moveToFirst() && cursor.count > 0) {
    ...
    companion object {
        ...
        const val CREATE_TABLE_SETTINGS =
            "create table $NAME_SETTINGS(_id integer primary key autoincrement,name varchar(20) not null,value text not null)"
        //endregion
