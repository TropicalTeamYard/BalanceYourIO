...
object BillRecordsProvider {
    ...
    fun getBillRecordsByTimeMode(
        billRecords: ArrayList<BillRecord>,
        timeMode: TimeMode
                        ...
                        if (DateConverter.equalDate(billRecords[j].time!!, timeModeList[i])) {
                            ...
                            when (billRecords[j].ioType) {
                                IOType.Income -> timeModeSum[i][IOType.Income] =
                                    timeModeSum[i][IOType.Income]!! + billRecords[j].amount
                                IOType.Outcome -> timeModeSum[i][IOType.Outcome] =
                                    timeModeSum[i][IOType.Outcome]!! + billRecords[j].amount
                                else -> timeModeSum[i][IOType.Unset] =
                                    timeModeSum[i][IOType.Unset]!! + billRecords[j].amount
                            }
                        }
                    }
