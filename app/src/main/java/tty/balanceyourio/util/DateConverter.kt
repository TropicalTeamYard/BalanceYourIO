...
object DateConverter {
    ...
    fun getFriendDateString(d: Date): String {
        ...
        str =
            "${calendar.get(Calendar.MONTH) + 1}月${calendar.get(Calendar.DAY_OF_MONTH)}日 星期${dArray[calendar.get(
    ...
    fun getSimpleDate(s: String): Date {
        ...
        if (time.size == 2) {
            ...
            try {
                ...
                calendar.time = d
                calendar.set(
                    Calendar.HOUR_OF_DAY, when (time[1]) {
                        "上午" -> 8
                        "中午" -> 11
                        "下午" -> 15
                        "晚上" -> 21
        ...
        return when (timeMode) {
            TimeMode.Day -> {
                if (calendar.get(Calendar.YEAR) == cYear) {
                    ...
                    "${calendar.get(Calendar.YEAR) % 10}.${calendar.get(Calendar.MONTH) + 1}.${calendar.get(
